...
4. If you care about AST nodes identity, a mapping from gast nodes to standard library
   nodes can be generated by using the function ``gast.parse_and_map``. This function returns
   a tuple which first element is the ``gast`` module and the second element is a 
   mapping from gast nodes to standard library nodes. This feature should be used with caution
   since not all nodes have a corespondance. Namely, ``expr_context`` nodes and the store ``Name`` of 
   ``ExceptHandler``s are not present in the mapping, there are more exceptions, especially in Python 2.
...
``ast.Param`` context. Additionnaly, the ``ExceptHandler.name`` attribute is turned into
an actual ``ast.Name`` instance with a ``Store`` context instead of simply being a string.
